...
const copy = {
  en: {
    ...
    footer_line: 'Rooted by nature. Chosen by you.', mockup_note: 'Review version — sample pieces, prices and contact details are placeholders. Product images are AI visualisations.',
  ...
  th: {
    ...
    footer_line: 'ธรรมชาติสร้าง คุณเป็นผู้เลือก', mockup_note: 'เวอร์ชันสำหรับรีวิว — ชิ้นงาน ราคา และช่องทางติดต่อเป็นข้อมูลตัวอย่าง ภาพสินค้าเป็นภาพจำลอง',
...
const content = {
  en: {
    ...
    visit: { eyebrow: 'VISIT THE WAREHOUSE', title: 'Walk among hundreds of singular forms.', intro: 'Bring your room dimensions, photographs and an open mind. We will help you find the piece that belongs there.', details: 'Plan your visit', hours: 'Opening hours', hoursText: 'Monday–Saturday\n09:00–17:00', address: 'Warehouse', addressText: 'Bangkok Metropolitan Area\nThailand', contact: 'Contact', contactText: contactLines('LINE and phone\nannounced at launch'), appointment: 'Before you come', appointmentText: 'Please contact us one day ahead so we can prepare the pieces you would like to see.', map: 'Warehouse location', mapNote: 'Mockup location — replace with the final address and map link.' }
  },
  th: {
    home: {
      ...
      manifestoLabel: 'มุมมองของเรา', manifesto: 'ไม่มีรุ่นให้สั่งซ้ำ และไม่มีฝาแฝด ทุกชิ้นของ RAAKRAK คือการพบกันเพียงครั้งเดียว ระหว่าง <em>รูปทรงเดิมของธรรมชาติ</em> กับพื้นที่ที่คุณเรียกว่าบ้าน', featured: 'ชิ้นงานที่มีตัวตน', featuredNote: 'ตัวอย่างรูปทรง 12 ชิ้นจากคลังหลายร้อยชิ้น แต่ละรหัสหมายถึงสินค้าจริงเพียงหนึ่งชิ้น',
    ...
    visit: { eyebrow: 'เยี่ยมชมโกดัง', title: 'เดินท่ามกลางรูปทรงนับร้อยที่ไม่ซ้ำกัน', intro: 'นำขนาดห้อง ภาพถ่าย และใจที่เปิดกว้างมา เราจะช่วยคุณหาชิ้นที่เหมาะกับพื้นที่จริง', details: 'วางแผนการเยี่ยมชม', hours: 'เวลาเปิด', hoursText: 'จันทร์–เสาร์\n09:00–17:00 น.', address: 'โกดัง', addressText: 'เขตกรุงเทพมหานครและปริมณฑล\nประเทศไทย', contact: 'ติดต่อ', contactText: contactLines('ไลน์และเบอร์โทร\nจะแจ้งเมื่อเปิดร้าน'), appointment: 'ก่อนเดินทาง', appointmentText: 'กรุณาติดต่อเราล่วงหน้าหนึ่งวัน เพื่อเตรียมชิ้นงานที่คุณสนใจไว้ให้ชม', map: 'ตำแหน่งโกดัง', mapNote: 'ตำแหน่งตัวอย่าง — เปลี่ยนเป็นที่อยู่และลิงก์แผนที่จริงภายหลัง' }
  }
};
...
function itemPage(code) {
  ...
  const gallery = [item.hero, ...item.photos];
  ...
  return `<article class="page item-page">
    ...
    <section class="item-gallery" aria-label="Product gallery">${gallery.map((src, index) => `<button class="gallery-image reveal" type="button" data-gallery-index="${index}">${image(src, `${item.code} — ${index === 0 ? tr('visualisation') : tr('warehouse_photo')} ${index + 1}`)}</button>`).join('')}</section>
...
function bindItem(code) {
  ...
  state.lightboxImages = [item.hero, ...item.photos];
